...
@implementation SleepTimeDrawView
{
...
-(void)startDraw:(NSMutableArray<StagingSubObj *> * ) sleepObjArray
...
-(void)drawTime:(NSMutableArray<StagingSubObj *> * ) sleepObjArray SleepStart:(NSNumber *)  startSleepTimeInterval  SleepEnd:(NSNumber *)endSleepTimeInterval
{
...
//    CGFloat layerW = self.drawView.bounds.size.width;
    ...
    gradientLayer.colors = @[ (__bridge id)[SleepPercentView colorWake].CGColor,
                               (__bridge id)[SleepPercentView colorREMSleep].CGColor,
                               (__bridge id)[SleepPercentView colorLightSleep].CGColor,
                               (__bridge id)[SleepPercentView colorDeepSleep].CGColor,
    ];
...
//    _wakeLayer = [self addLayerToDrawView:CGRectMake(0, layerStartY, layerW, SLEEP_LAYER_HEIGHT)];
    
//    _wakeLayer.backgroundColor = UIColor.redColor.CGColor;
    
//    _remLayer = [self addLayerToDrawView:CGRectMake(0, layerStartY + SLEEP_LAYER_HEIGHT, layerW, SLEEP_LAYER_HEIGHT)];
//    _remLayer.backgroundColor = UIColor.blueColor.CGColor;

//    _lightLayer = [self addLayerToDrawView:CGRectMake(0, layerStartY + SLEEP_LAYER_HEIGHT *2, layerW, SLEEP_LAYER_HEIGHT)];
//    _lightLayer.backgroundColor = UIColor.greenColor.CGColor;

//    _deepLayer = [self addLayerToDrawView:CGRectMake(0, layerStartY + SLEEP_LAYER_HEIGHT *3, layerW, SLEEP_LAYER_HEIGHT)];
//    _deepLayer.backgroundColor = UIColor.grayColor.CGColor;
    ...
    for (int i = 0; i < sleepObjArray.count; i++) {
        StagingSubObj *sleepObj = sleepObjArray[i];

//        SleepObj *sleepObj = sleepObjArray[i];
        // 此段睡眠的时间
        NSNumber *tempStartTime = sleepObj.list.firstObject.time;
        NSNumber *tempEndTime;
        if (i < sleepObjArray.count - 1) {
            StagingSubObj *nexSleepObj = sleepObjArray[i + 1];

            tempEndTime = nexSleepObj.list.firstObject.time;
        } else {
            tempEndTime = sleepObj.list.lastObject.time;
        }
        
        // 确定在哪个layer
        NSNumber *tempStartY =  [self chooseStartY:sleepObj.type];
        ...
        UIColor *color = [self chooseColor:sleepObj.type]; // 方块颜色
...
//    [lineBez addArcWithCenter:CGPointMake(gradientLayer.bounds.size.width /2, gradientLayer.bounds.size.height /2) radius:20 startAngle:0 endAngle:M_PI * 2 clockwise:YES];
    ...
    for (int i = 0; i < layerArray.count; i++) {
        ...
        if (i < layerArray.count - 1) {
            // 画连线
            CALayer *nexlayer = layerArray[i+1];
            
    //        UIBezierPath *tempBez = [UIBezierPath bezierPath];
            CGPoint point_one, point_two, point_three, point_four;
            
    //        NSLog(@"layer x=%f y=%f w=%f h=%f", curlayer.frame.origin.x, curlayer.frame.origin.y, curlayer.frame.size.width, curlayer.frame.size.height);
            if (nexlayer.frame.origin.y > curlayer.frame.origin.y) {
                // 下一个比它低
                point_one = CGPointMake(CGRectGetMaxX(curlayer.frame) , curlayer.frame.origin.y - layerStartY);
                point_two = CGPointMake(CGRectGetMaxX(curlayer.frame)+ diff, curlayer.frame.origin.y - layerStartY);
                point_three = CGPointMake(nexlayer.frame.origin.x+diff, CGRectGetMaxY(nexlayer.frame) - layerStartY);
                point_four =  CGPointMake(nexlayer.frame.origin.x, CGRectGetMaxY(nexlayer.frame) - layerStartY);
            ...
            else
            {
                ...
                point_one = CGPointMake(CGRectGetMaxX(curlayer.frame), curlayer.frame.origin.y  - layerStartY);
                point_two = CGPointMake(CGRectGetMaxX(curlayer.frame) - diff , curlayer.frame.origin.y  - layerStartY);
                point_three = CGPointMake(nexlayer.frame.origin.x - diff , nexlayer.frame.origin.y - layerStartY);
                point_four = CGPointMake(nexlayer.frame.origin.x , nexlayer.frame.origin.y - layerStartY);
            }
            [lineBez moveToPoint:point_one];
            [lineBez addLineToPoint:point_two];
            [lineBez addLineToPoint:point_three];
            [lineBez addLineToPoint:point_four];
        }
